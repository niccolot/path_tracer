...
void test() {
    ...
    auto sphere2 = std::make_shared<Lambertian>(Color(0.8,0.3,0.4));
    auto sphere3 = std::make_shared<Dielectric>(1.5);
    auto sphere4 = std::make_shared<Dielectric>(1.5);
    ...
    world.add(std::make_shared<Sphere>(Vec3(0,0,-5), 7, sphere2));
    world.add(std::make_shared<Sphere>(Vec3(1.5,0,3), 0.5, sphere3));
    world.add(std::make_shared<Sphere>(Vec3(-1.5,0,3), 0.5, sphere4));

    HittableList lights;
    auto empty = std::shared_ptr<Material>();
    lights.add(std::make_shared<Quad>(Vec3(0.5,20,0.5), Vec3(-0.5,0,0), Vec3(0,0,-0.5), empty));
    lights.add(std::make_shared<Sphere>(Vec3(0,0,3), 0.5, empty));
    lights.add(std::make_shared<Sphere>(Vec3(0,-100.5,-1), 100, empty));
    lights.add(std::make_shared<Sphere>(Vec3(0,0,-5), 7, empty));
    lights.add(std::make_shared<Sphere>(Vec3(1.5,0,3), 0.5, empty));
    lights.add(std::make_shared<Sphere>(Vec3(-1.5,0,3), 0.5, empty));
    Camera cam(400, 16./9.,Vec3(0,0,5),Vec3(0,0,0),90);
    cam.render(world, lights);
}
